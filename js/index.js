...
const allMonth = ['январь', 'февраль', 'март', 'апрель', 'май', 'июнь', 'июль', 'август', 'сентябрь', 'октябрь', 'ноябрь', 'декабрь'];

const showResultData = () => {
  const currentYear = new Date().getFullYear();
  const monthIndex = allMonth.findIndex(item => item === month);
  const dateString = `${currentYear}-${(monthIndex + 1).toString().padStart(2, '0')}-${
...
const createFormMonth = (months) => {
  ...
  const nextMonths = months.filter((item, index) => {
    return index >= currentMonthIndex && index < currentMonthIndex + 3;
  });

  const buttonsMonth = nextMonths.map(item => {
    const btn = document.createElement('button');
    btn.className = 'form__btn-month';
    btn.type = 'button';
    btn.textContent = `${item[0].toUpperCase()}${item.slice(1).toLowerCase()}`;
...
const init = () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch('https://imaginary-mirage-school.glitch.me/api/orders', {
        method: 'POST',
        headers: {
